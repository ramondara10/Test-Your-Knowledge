...
var correctAnswers = {
    ...
    answer4: questions[0].answer,
    answer5: questions[0].answer,
}
...
function renderPreviousPlayer() {
    ...
    if (previousPlayer == null || previousScore === null) {
        ...
        document.querySelector("#highscore").textContent = `${previousPlayer}:${previousScore}`
    }
}
...
function startQuiz(event) {
    ...
    document.querySelector("#startjsbtn").addEventListener("click", function (event) {
        event.stopPropagation();
        score = 0;
        // function ot make the time go backwards
        var interval = setInterval(function () {
            timeLeft--;
            document.querySelector("#count").textContent = "Time left:" + timeLeft;
            if (timeLeft === 0) {
                ...
                document.querySelector("#count").textContent = "Time left: 0";
                document.querySelector("#score").textContent = "Here is your score:" + score;
        ...
        introOutro.textContent = questions[0].title;
        ...
        for (let i = 0; i < questions[0].choices.length; i++) {
            ...
            newBtn.textContent = question[0].choices[i]
        ...
        container.addEventListener("click", function (event) {
            ...
            if (event.target.matches(".choicesbtn")) {
                ...
                if (click === 1) {
                    ...
                    for (var i = 0; i < questions[click].choices.length; i++) {
                        document.querySelector(btnID[i].textContent = questions[click].choices[i])
                    }
                }
                if (click === 2) {
                    introOutro.textContent = questions[click].title;
                ...
                if (click > 4) {
                    ...
                    setTimesUp(function () {
                        playerName = prompt("Please type your initials to save your score")
                        playerScore = score;
                        localStorage.setItem("playername", playerName)
                ...
                //confirm choices
                if (userchoice === correctAnswers.answer1 || userchoice === correctAnswers.answer2 || userchoice === correctAnswers.answer3 || userchoice === correctAnswers.answer4 || userchoice === correctAnswers.answer5) {
                    score = score + 20;
